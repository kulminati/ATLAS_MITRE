...
type TabKey = "learn" | "subtechniques" | "mitigations" | "case_studies" | "osint";
...
export default function TechniqueDetailTabs({ technique }: Props) {
  const allTabs: TabDef[] = [
    {
      key: "learn" as const,
      label: "Learn",
      count: null, // always show - content loaded on demand
    },
    {
  ...
  const [activeTab, setActiveTab] = useState<TabKey>(
    tabs.length > 0 ? tabs[0].key : "learn"
  );
  ...
  return (
    <div>
        ...
        {tabs.map((tab) => (
          ...
          >
            ...
            {tab.key === "learn" && (
              <svg
                className={`w-4 h-4 ${
                  activeTab === tab.key ? "text-blue-300" : "text-blue-500"
                }`}
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
      ...
      <div className="space-y-2">
        {activeTab === "learn" && (
          <DeepDivePanel techniqueId={technique.id} />
        )}
